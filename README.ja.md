# 秘封倶楽部

[English](README.md) | [简体中文](README.zh-CN.md) | [日本語](README.ja.md)

秘封倶楽部のための非公式ファンメイド読書室です。

このサイトは、音楽と文章のあいだにある不思議な時間のために作られています。アルバムを開き、曲を再生し、その横でブックレットの物語を読む。各アルバムには専用ページがあり、各トラックは対応する物語セクションと結び付けられます。読者は中国語と日本語を切り替えながら、静かな秘封アーカイブを歩くことができます。

## このサイトについて

- ZUN's Music Collection と秘封倶楽部ストーリーのための静的ファンサイト
- ブックレットストーリー付きの秘封音楽 CD 9 作品に対応
- 固定プレイヤーと、曲に連動する物語セクションのハイライト
- 中国語 / 日本語の読書モード
- バックエンド不要、GitHub Pages で公開可能

## 対象アルバム

現在、以下のアルバム用コンテンツファイルがあります。

```text
蓮台野夜行
夢違科学世紀
卯酉東海道
大空魔術
鳥船遺跡
伊弉諾物質
燕石博物誌
旧約酒場
七夕坂夢幻能
```

## 物語と音声の追加

アルバム本文ファイルはここにあります。

```text
content/albums/
```

音声ファイルは次の形式で参照します。

```text
assets/audio/<album-id>/01.mp3
```

対応するアルバム JSON を編集し、`text.zh` と `text.ja` を入力してください。`track` は曲、音声ファイル、物語セクションを結び付けます。

```json
{
  "track": 1,
  "title": { "zh": "中文标题", "ja": "日本語タイトル" },
  "text": { "zh": "中文正文", "ja": "日本語本文" }
}
```

音声ファイルは標準で `.gitignore` により無視されます。GitHub Pages 上でも音声を再生したい場合は、該当する音声ファイルの ignore ルールを削除してから commit してください。

## ローカルプレビュー

```sh
python3 -m http.server 4173 --bind 127.0.0.1
```

その後、以下を開きます。

```text
http://127.0.0.1:4173
```

## GitHub Pages

このリポジトリには GitHub Pages 用の自動デプロイ workflow が含まれています。`main` に push したあと、リポジトリ設定で以下を確認してください。

```text
Settings -> Pages -> Build and deployment -> Source -> GitHub Actions
```

## 注意

これは読書体験と整理のためのファンプロジェクトです。使用権のあるテキストと音声のみ追加してください。
